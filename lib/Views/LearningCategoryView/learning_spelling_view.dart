...
class LearningSpellingView extends StatefulWidget {
  const LearningSpellingView({
    required this.items,
  ...
  @override
  State<LearningSpellingView> createState() => _LearningSpellingViewState();
}

class _LearningSpellingViewState extends State<LearningSpellingView> {
  ...
  Future<void> _saveProgress({bool? lastAnswerCorrect}) {
    return saveLessonProgress(
      ...
      routeName: '/${widget.lessonSlug}/spelling',
  ...
  void _buildChoices({int? targetIndex}) {
    ...
    final target = widget.items[nextIndex].name;
    final choices = <String>{target};
    final maxChoices = widget.choiceCount.clamp(2, widget.items.length);

    while (choices.length < maxChoices) {
      choices.add(widget.items[_random.nextInt(widget.items.length)].name);
    }
  ...
  void _selectChoice(String choice) {
    final target = widget.items[_currentIndex].name;
  ...
  Widget build(BuildContext context) {
    final target = widget.items[_currentIndex];
    final crossAxisCount = _choices.length <= 4 ? 2 : 4;
            ...
            children: [
              ...
              GridView.builder(
                ...
                gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(
                  ...
                  crossAxisSpacing: 12,
                  mainAxisSpacing: 12,
                  childAspectRatio: crossAxisCount == 2 ? 2.2 : 1.05,
                ),
