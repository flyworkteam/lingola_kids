...
class LearningTrueFalseView extends StatefulWidget {
  const LearningTrueFalseView({
    required this.items,
  ...
  @override
  State<LearningTrueFalseView> createState() => _LearningTrueFalseViewState();
}

class _LearningTrueFalseViewState extends State<LearningTrueFalseView> {
  final math.Random _random = math.Random();

  late int _askedIndex = widget.initialIndex;
  late int _shownIndex = widget.initialIndex;
  ...
  Future<void> _saveProgress({bool? lastAnswerCorrect}) {
    return saveLessonProgress(
      ...
      routeName: '/${widget.lessonSlug}/true-false',
      currentItemIndex: _askedIndex,
      totalItems: widget.items.length,
      currentItemKey: widget.items[_askedIndex].name.replaceAll(' ', '-'),
  ...
  void _buildQuestion({int? targetIndex}) {
    ...
    final canShowWrongItem = widget.items.length > 1;
    final shouldShowCorrectItem =
        !canShowWrongItem || _nextQuestionShouldShowCorrect;
    final nextShownIndex = shouldShowCorrectItem
        ? nextAskedIndex
  ...
  int _randomDifferentIndex(int excludedIndex) {
    var index = _random.nextInt(widget.items.length - 1);
  ...
  void _answer(bool answer) {
    ...
    recordLessonEvent(
      ...
      itemKey: widget.items[_askedIndex].name.replaceAll(' ', '-'),
  ...
  Future<void> _handleResultContinue() async {
    if (_isCorrect) {
      final nextIndex = (_askedIndex + 1) % widget.items.length;
  ...
  Widget build(BuildContext context) {
    final askedItem = widget.items[_askedIndex];
    final shownItem = widget.items[_shownIndex];
    final subjectText = widget.subject.isEmpty ? '' : '${widget.subject} ';
